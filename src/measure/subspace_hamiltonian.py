...
def _interaction_from_fermion_operator(num_spin_orbitals: int):
    ...
    interaction = get_interaction_operator(fermion_operator)
    return interaction
...
def run_sqd_from_counts(
    ...
    *,
    ...
    hcore = np.asarray(interaction.one_body_tensor).real
    eri = np.asarray(interaction.two_body_tensor).real
    nuclear_repulsion_energy = float(np.real(interaction.constant))
    spin_value = spin / 2.0
    ...
    for i in range(iterations):
        ...
        if avg_occupancy is None:
            ...
            try:
                ...
                print(f"  WARNING: Configuration recovery failed: {err}")
                print(f"  Using fall-back configuration (full set).")
                bs_mat_tmp = bitstring_matrix_full
                probs_arr_tmp = probs_arr_full
